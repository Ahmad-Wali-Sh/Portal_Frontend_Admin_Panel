...
import {
  ...
  Pencil, Loader2, AlertCircle, X, ChevronUp, ChevronDown as ChevronDownIcon,
} from 'lucide-react'
            ...
            .then((res) => {
              ...
              setOptions(
                raw.map((item) => ({
                  ...
                  label: typeof filter.fetchOptions.labelKey === 'function'
                    ? filter.fetchOptions.labelKey(item)
                    : item[filter.fetchOptions.labelKey ?? 'name'] ?? item.id,
                }))
              )
            })
            .catch(() => {})
...
export default function MasterList({ config, isDark, onEdit }) {
  ...
  }, [page, search, filters, orderBy, config.apiPath, limit]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const SortIcon = ({ col }) => {
    ...
    return (
      <span className="ml-1 inline-flex flex-col gap-0">
        <ChevronUp size={9} className={cn(isActive && currentDir === 'asc' ? 'text-primary' : 'text-gray-400 opacity-50')} />
        <ChevronDownIcon size={9} className={cn(isActive && currentDir === 'desc' ? 'text-primary' : 'text-gray-400 opacity-50')} />
      </span>
    )
  }
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-3">
          ...
          {hasFilters && (
            ...
            >
              ...
              Filters
              {activeFilterCount > 0 && (
                <span className="badge badge-blue text-[10px] px-1.5 py-0">{activeFilterCount}</span>
              )}
            </button>
          )}
          ...
          <span className="ml-auto text-xs text-muted-foreground whitespace-nowrap">
            {meta.total} {config.entityName ?? 'records'}{meta.total !== 1 ? 's' : ''}
          </span>
        </div>
        ...
        {filtersOpen && hasFilters && (
          <div className={cn(
            ...
            isDark ? 'bg-gray-800/60 border-gray-700' : 'bg-muted/50 border-border'
          )}>
            <FilterBar
