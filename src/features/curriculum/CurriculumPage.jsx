...
export default function CurriculumPage() {
  ...
  const curriculumConfig = {
    ...
    fields: [
      {
        key: 'name', label: 'Curriculum Name', type: 'text', required: true,
        placeholder: 'e.g. Business English – Cycle 12',
      },
      {
        key: 'cycle_id', label: 'Cycle', type: 'select', required: true,
        fetchOptions: { url: '/api/cycles', valueKey: 'id', labelKey: 'name' },
      },
      {
        key: 'class_id', label: 'Class (optional)', type: 'select', required: false,
        fetchOptions: { url: '/api/classes', valueKey: 'id', labelKey: 'name', params: { limit: 0 } },
        placeholder: 'Leave empty for cycle‑level template',
      },
      {
        key: 'employee_id', label: 'Responsible Employee', type: 'select', required: true,
        fetchOptions: {
          url: '/api/employees', valueKey: 'id',
          labelKey: (emp) => `${emp.name} ${emp.lastname || ''}`.trim(),
        },
      },
    ],
